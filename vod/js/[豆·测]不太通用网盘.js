...
async function getClassList(args) {
    ...
    backData.data = [];
    const existingIds = new Set(); // 用于去重的 Set

    try {
        const homeUrl = appConfig.webSite;
        const pro = await req(homeUrl);
        if (pro.data) {
            const $ = cheerio.load(pro.data);
            
            // 仅提取主导航栏的分类（排除下拉菜单）
            $('.nav-menu-item a[href^="/index.php/vod/type/id/"]').each((_, element) => {
                const $link = $(element);
                const href = $link.attr('href');
                const typeIdMatch = href.match(/\/id\/(\d+)\.html/);
                const typeName = $link.text().trim();

                if (typeIdMatch && typeIdMatch[1] && !existingIds.has(typeIdMatch[1])) {
                    backData.data.push({
                        type_id: typeIdMatch[1],
                        type_name: typeName,
                        hasSubclass: false
                    });
                    existingIds.add(typeIdMatch[1]); // 标记已存在的 ID
                }
            });
        }
    } catch (error) {
        backData.error = "自动获取分类失败：" + error.message;
    }
